...
import { useVaultingPools, usePendingRewardVaulCount } from '../../../state/vaults/hooks';
...
const VaulHarvestAll: React.FC = () => {
  ...
  const numberVaulExistReward = usePendingRewardVaulCount();
  ...
  const onHarvest = useCallback(async () => {
    ...
    try {
      const tx = await handleTransactionReceipt(
        `Claim ${rewardToken?.symbol} in ${numberVaulExistReward} ${
          numberVaulExistReward === 1 ? 'pool' : 'pools'
        }`,
        harvestAllRewards,
      );
      if (tx) {
    ...
    <StyledContainer>
      ...
      {/* <StyledReward>
        ...
        <Button
          ...
          onClick={onHarvest}
        >
          {`Claim All`} {numberVaulExistReward ? `(${numberVaulExistReward})` : []}
        </Button>
      </StyledReward> */}
    </StyledContainer>
  );
